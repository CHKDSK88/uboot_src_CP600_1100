...
# error Unknown CPU type
...

	{
		...
		char *upgradeFlagEnvVar;
		
		upgradeFlagEnvVar = getenv ("upgradeFlag");
		if (upgradeFlagEnvVar != NULL)
		{
			if (strncmp (upgradeFlagEnvVar,"1",1) == 0)
			{
				/* configure HW watchdog to start counting in order to monitor upgrade process */
				mv_config_hwwatchdog(0xffffffff,1);
				/* verify the values and print */
				mv_read_hwwatchdog(&time, &start);
				printf("HW watchdog start 0x%x %d\n",time,(start!=0));
			}
		}
	}
