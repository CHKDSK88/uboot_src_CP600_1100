...
extern void cp_USB_LED_set(int portnr, int on, int blink);
...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	if (buffer[DOS_PART_MAGIC_OFFSET] != 0x55 ||
		...
		/* no signature found */
		printf("no DOS partition signature found \n");
		free(buffer);
		return -1;
	}
...
extern int g_should_pause_after_usb_install_firm;
	     ...
	     int dols)
{
    ...
    mydata->sector_size = le16_to_int(bs.sector_size);					  
...
/* Check Point fw1*.img wildcard starts here */ 	
 
	    //printf ("before strcmp fnamecopy = %s sname = %s  lname = %s !! \n", fnamecopy, s_name, l_name);       
		if ( l_name[0] != '\0')
		{
			//printf("checking l_name %s and filename %s\n", l_name, filename);
			
			if (cp_filename_like(filename, CP_IMAGE_PREFIX, CP_IMAGE_SUFFIX) &&
				cp_filename_like(l_name, CP_IMAGE_PREFIX, CP_IMAGE_SUFFIX))
			{
				printf("Found image file: %s\n", l_name);
				file_not_found = 0;
				if (! strncmp(RAPID_DEP_CP_IMAGE_PREFIX, l_name, strlen(RAPID_DEP_CP_IMAGE_PREFIX))) {
					g_should_pause_after_usb_install_firm=0;	
				} else {
					g_should_pause_after_usb_install_firm=1;
				}
				goto rootdir_done; 
			}
			if (cp_filename_like(filename, CP_UBOOT_PREFIX, CP_UBOOT_SUFFIX) &&
				cp_filename_like(l_name, CP_UBOOT_PREFIX, CP_UBOOT_SUFFIX) )
			{
				printf("Found uboot file: %s\n", l_name);
				file_not_found = 0;
				goto rootdir_done; 
			}
			if (cp_filename_like(filename, CP_DSL_IMAGE_PREFIX, CP_IMAGE_SUFFIX) &&
				cp_filename_like(l_name, CP_DSL_IMAGE_PREFIX, CP_IMAGE_SUFFIX) )
			{
				printf("Found dsl image file: %s\n", l_name);
				file_not_found = 0;
				goto rootdir_done; 
			}
		}
		...
		{
			//printf("checking s_name %s and filename %s\n", l_name, filename);
			
			if (cp_filename_like(filename, CP_IMAGE_PREFIX, CP_IMAGE_SUFFIX) &&
				cp_filename_like(s_name, CP_IMAGE_PREFIX, CP_IMAGE_SUFFIX))
			{
				printf("Found image file: %s\n", l_name);
				file_not_found = 0;
				if (! strncmp(RAPID_DEP_CP_IMAGE_PREFIX, l_name, strlen(RAPID_DEP_CP_IMAGE_PREFIX))) {
			...
			if (cp_filename_like(filename, CP_UBOOT_PREFIX, CP_UBOOT_SUFFIX) &&
				cp_filename_like(s_name, CP_UBOOT_PREFIX, CP_UBOOT_SUFFIX) )
			{
				printf("Found uboot file: %s\n", l_name);
				file_not_found = 0;
				goto rootdir_done; 
			...
			if (cp_filename_like(filename, CP_DSL_IMAGE_PREFIX, CP_IMAGE_SUFFIX) &&
				cp_filename_like(s_name, CP_DSL_IMAGE_PREFIX, CP_IMAGE_SUFFIX))
			{
				printf("Found dsl image file: %s\n", l_name);
				file_not_found = 0;
				goto rootdir_done; 
...
/* Check Point fw1*.img wildcard ends here  */
...
{
	printf("Looking for file_fat_read %s\n",filename);
