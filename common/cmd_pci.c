...
static int is_wireless_exist(int BusNum)
{
	int Device;
	int Function;
	unsigned char HeaderType;
	...
	for (Device = 0; Device < PCI_MAX_PCI_DEVICES; Device++) {
		...
		for (Function = 0; Function < PCI_MAX_PCI_FUNCTIONS; Function++) {
...

			{
				...
				if (vendor == 0x168c && device == 0x002e) {
...
int do_pci (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	case 'r': /* restart */
		...
		if ((is_wireless_supported()) && (! is_wireless_exist(0))) {
			int i;
			
			for (i=0; i < 10; i++) {
				printf("Restarting PCI(%d)...", i+1);
				extend_gpio_ctl(IC_SG80_EXTEND2,6,0);
				udelay(5000000);
				extend_gpio_ctl(IC_SG80_EXTEND2,6,1);
				printf("Done.\n");
				udelay(5000000);
				if (is_wireless_exist(0)) {
					break;
				}
			}
		}
