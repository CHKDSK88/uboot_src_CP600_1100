...
static int flash_io (int mode)
{
	...
	if (mode == O_RDWR) {
        ...
        /* Issue with Writing incorrect page size to NAND
        ...
        /* Writing Bootloader Env to NAND by Rounding off Length
         * envBuffLen is CFG_ENV_SIZE
         * Redund Env not defined
         */
        envBuffLen = (len + ENV_SIZE);
        printf("Buffer size for Env %d\n", envBuffLen);
        envCrcData = malloc(envBuffLen);
        if (envCrcData){
            printf("Buffer allocated for Env \n");
        if( memcpy(envCrcData, &environment.crc, len) != envCrcData) {
            fprintf(stderr, "Env CRC to Buffer write error \n");
			return (-1);
            }
        if (memcpy(envCrcData + len, environment.data, ENV_SIZE) != envCrcData +
            len) {
            fprintf(stderr, "Env Data to Buffer write error \n");
			return (-1);
            }
        }else{
            fprintf(stderr, "Env Buffer allocation error \n");
			return (-1);
		}
		if (write (fdr, envCrcData , envBuffLen) != envBuffLen) {
			fprintf (stderr,
				"Write error on %s: %s\n",
				DEVNAME (otherdev), strerror (errno));
			return (-1);
