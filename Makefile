...
dramregs_$(MV_DDR_FREQ)_A.txt u-boot-${MV_OUTPUT}.bin u-boot-${UV_BUILDNUMBER}_$(MV_DDR_FREQ)_nand.bin
DO_IMAGE_UART = ./tools/doimage -T uart -D 0x600000 -E 0x670000 -R \
dramregs_$(MV_DDR_FREQ)_A.txt u-boot-${MV_OUTPUT}.bin u-boot-${UV_BUILDNUMBER}_$(MV_DDR_FREQ)_uart.bin
DO_IMAGE_FLASH = ./tools/doimage -T flash -D 0x600000 -E 0x670000 -R \
dramregs_$(MV_DDR_FREQ)_A.txt u-boot-${MV_OUTPUT}.bin u-boot-${UV_BUILDNUMBER}_$(MV_DDR_FREQ)_flash.bin
...
mv_kw:	unconfig
	...
	@chmod +w board/mv_feroceon/config_kw/config.mk;
...
#=======================
	...
	@[ -z "$(findstring rd88f6281a_config,$(RULE))" ] || \
		{ echo "MV_FLAGS += -DRD_88F6281A" >> include/config.mk;  \
		  ...
		  echo "MV_FLAGS += -DMV_BOOTROM" >> include/config.mk;	\
		  cp board/mv_feroceon/config_kw/u-boot-sec256k.lds board/mv_feroceon/config_kw/u-boot.lds;	\
		  chmod +w board/mv_feroceon/config_kw/u-boot.lds; \
...
# SEATTLE /LONDON
#==============================
ifeq ($(SUB_HW_VER),seattle)
	@echo  "MV_DDR_FREQ = seattle_400rd" >> include/config.mk;
	echo "#define SEATTLE"	>>include/config.h;
else ifeq ($(SUB_HW_VER),london)
	@echo  "MV_DDR_FREQ = london_400rd" >> include/config.mk;
	echo "#define LONDON"	>>include/config.h;
endif
	echo "#define BUILD_NUMBER $(BUILD_NUMBER)" >> include/config.h;
...
ifeq ($(NBOOT),1)
	...
	@chmod +w board/mv_feroceon/config/u-boot.lds;
else
...
clobber:	clean
	find . -type f \( -name .depend \
		-o -name '*.srec' -o -name '*$(SUB_HW_VER)*.bin' -o -name u-boot.img \) \
		-print0 \
		| xargs -0 rm -f
