...
void common_err(ulong *adr, ulong good, ulong bad, ulong xor, int type) 
{
	...
	{	/* Don't do anything for a parity error. */
		...
		v->erri.tbits += n;
		if (n > (unsigned int)v->erri.max_bits) 
		{
			v->erri.max_bits = n;
			flag++;
		}
		if (n < (unsigned int)v->erri.min_bits) 
		{
			v->erri.min_bits = n;
			flag++;
		}
		...
		if (len > (unsigned int)v->erri.maxl) 
		{
			v->erri.maxl = len;
			flag++;
		}
		...
		
		{
			...
			diag_printf("Current Detect Error On Address: %08x. bad data %08x\n", v->erri.eadr, bad);	
		}
