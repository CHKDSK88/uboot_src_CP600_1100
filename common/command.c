...
int do_passwd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
    {
    char user_buff[BUFFER_MAX_LEN] = "\0";    
    int status;
    char *passwd;
    if ((passwd = getenv ("password")) != NULL)
        {
        for(;;){
            memset(user_buff, 0, BUFFER_MAX_LEN);
            puts("\n");
            status = read_password("Enter password:", user_buff, BUFFER_MAX_LEN);
            if (status != -1){
                if (strcmp(user_buff,passwd) == 0){
                    /*Grant User Access*/
                    return 1;
                }
                else
...
U_BOOT_CMD( 
        passwd,	1,	0,	do_passwd,
        "passwd    - It will authenticate User",
        NULL
);
