...
unsigned int build_number = BUILD_NUMBER;
...
void start_armboot (void)
{
	...
	cp_GPIO_LED_set(POWER_LED_GREEN,LED_CMD_STATIC);
        cp_GPIO_LED_set(POWER_LED_GREEN,LED_CMD_ON);//HW REQUIRMENT: Power LED does not require initialization 
   ...
   if(cpu == 0)
...
#endif
	env = getenv("cp_boot_no_console_output");
	
	if (env && (strncmp(env, "1", 1)==0)) {
		
		extern int g_cp_boot_no_output;
		g_cp_boot_no_output=1;
		
		gd->flags |= GD_FLG_SILENT;
		setenv("cp_console_str", " ");
	} else {
		sprintf(tmp2, "ttyS0,%s",getenv("baudrate"));
		setenv("cp_console_str", tmp2);
	}
	init_baudrate();	
	serial_setbrg ();
	udelay(50000);
	...
	if (getenv("noExtPorts") && (! strncmp(getenv("noExtPorts"), "1", 1))) {
		is_ext_ports_disabled=1;
	}

	sprintf(tmp,"%d", BUILD_NUMBER);
	setenv("bootloader_version",tmp);
...
extern void twsi_init(void);
	twsi_init(); /* for SG80, do twsi init first */

#ifdef SEATTLE
	run_command("sx1505 0x21 gpio hi 3",0);/*  USB1 power OFF */
   	run_command("sx1505 0x21 gpio hi 4",0);/*  USB2 power OFF */
	/* Turn off leds */
	run_command("sx1505 0x20 gpio hi 7",0);/* Turn off wifi led */
	run_command("sx1505 0x20 gpio hi 1",0);/* Turn off internet led */
	run_command("sx1505 0x20 gpio hi 3",0);/* Turn off internet led */
	run_command("sx1505 0x20 gpio hi 0",0);/* Turn off USB1 led */
	run_command("sx1505 0x20 gpio hi 4",0);/* Turn off USB1 led */
	run_command("sx1505 0x20 gpio hi 2",0);/* Turn off USB2 led */
	run_command("sx1505 0x20 gpio hi 5",0);/* Turn off USB2 led */
	run_command("sx1505 0x21 gpio hi 2",0);/* Turn off Alert led */

	run_command("sx1505 0x21 gpio hi 5",0); /* wifi power down */
	run_command("sx1505 0x20 gpio low 6",0); /* reset low */
	udelay(100000);
	run_command("sx1505 0x21 gpio hi 5",0);  /* power up */
	udelay(100000);
	run_command("sx1505 0x20 gpio hi 6",0);  /* reset high */
		/* reset ADSL BOARD */
	run_command("sx1505 0x21 gpio low 0",0); /* DSL power down */
	run_command("sx1505 0x21 gpio low 6",0); /* DSL SW reset low */
	udelay(100000);
	run_command("sx1505 0x21 gpio hi 0",0);  /* power up */
	udelay(100000);
	run_command("sx1505 0x21 gpio hi 6",0);  /* reset high */
		/* reset exp Card */
	run_command("sx1505 0x21 gpio low 7",0); /* power down */
	run_command("sx1505 0x21 gpio low 1",0); /* reset low */
	udelay(100000);
   	if (! is_ext_ports_disabled) {
		run_command("sx1505 0x21 gpio hi 7",0);  /* power up */
	} 
	udelay(100000);
	run_command("sx1505 0x21 gpio hi 1",0);  /* reset high */
   /* Power on USB1 and USB2 */
   	if (! is_ext_ports_disabled) {
		run_command("sx1505 0x21 gpio low 3",0);/* Enable USB1 power */
		run_command("sx1505 0x21 gpio low 4",0);/* Enable USB2 power */	
	} else {
		run_command("sx1505 0x21 gpio high 3",0);/* Disable USB1 power */
		run_command("sx1505 0x21 gpio high 4",0);/* Disable USB2 power */	
	}
	/* set port6 of 2nd 6171R to 10/half mode */
	run_command("switch_mii_w 0x02 0x16 0x01 0xc0fd",0);
#endif
	/* Initialize LED and power values 
	 * Attention_G		- low	(Attention green LED on)
	 * Power status_R	- high	(Power red LED off) 
	 * Attention_R		- high	(Attention red LED off)
	 * Power status_G	- low	(Power green LED on)
	 * ADSL_ACT_G		- low	(ADSL green activity LED on)
	 * ADSL_LINK_G		- low	(ADSL green link LED on)
	 * PCIE_RST			- high
	 * value of register: 0b01000110 = 0x46;  
	 * IC_SG80_SX150X_ADDR=0x20; */
	/* TODO: add include and TO_STR macro for binary values here */
