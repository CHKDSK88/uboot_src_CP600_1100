...
int nand_erase_opts(nand_info_t *meminfo, const nand_erase_options_t *opts)
{
	...
	for (;
		...
		if (!opts->scrub && bbtest) {
			...
			if (ret > 0) {
				if (!opts->quiet)
					printf("\rSkipping Nand block at  "
					       "0x%08x                   "
					       "                         \n",
					       erase.addr);
				continue;
...
int nand_mark_bad(nand_info_t *nand, ulong addr) {
	...
	while (imglen && (mtdoffset < meminfo->size)) {
		...
		if (result != 0) {
			printf("writing NAND page at offset 0x%lx failed\n",
			       mtdoffset);
			if (nand_mark_bad(meminfo, mtdoffset))
				goto restoreoob;
			else {
				blockstart = -1;
				buffer = blockbuf;
...
int nand_read_skip_bad(nand_info_t *nand, size_t offset, size_t *length,
                       u_char *buffer, int quiet) {

	return nand_read_skip_bad_ex(nand, offset, length, buffer, quiet, NULL);
}

int nand_read_skip_bad_ex(nand_info_t *nand, size_t offset, size_t *length,
                       u_char *buffer, int quiet, size_t *last_offeset_ret)
{
    int rval;
    ...
    while (left_to_read > 0) {
            ...
            last_offset = offset + read_length;
            ...
            if (! quiet) {
                    int percent;
                    unsigned long long n = (unsigned long long)
                            ((total_length - left_to_read)) * 100;				
			
                    do_div(n, total_length);
                    percent = (int)n;
