...
static int usb_test_unit_ready(ccb *srb,struct us_data *ss)
{
	...
	do {
		...
		/*
		 * When a USB card reader is empty, 
		 * it will return "Not Ready - medium not present" 
		 * as Key Code Qualifier. In that situation, 
		 * it's useless waiting for the full timeout since 
	   	 * the result won't change until the user inserts a card.
		 */
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
	...
	if (dev_desc->blksz > MAX_SECTOR_SIZE) //maximal supported block size is 4096
		return 0;
	dev_desc->type = perq;
	USB_STOR_PRINTF(" address %d\n",dev_desc->target);
