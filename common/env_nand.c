...
{
	ulong total;
	int ret = 0, eccEnv = 0, i = 0, goodBlockCounter = 0;
	...
	if (nand_erase(&nand_info[0], i * nand_info[0].erasesize, CFG_ENV_SIZE))
#else
	...
	total = CFG_ENV_SIZE;
#ifndef MV78XX0
	ret = nand_write(&nand_info[0], i * nand_info[0].erasesize, &total, (u_char*)env_ptr);
#else
...
void env_relocate_spec (void)
{
#if !defined(ENV_IS_EMBEDDED)
	ulong total;
	int ret, i = 0, goodBlockCounter = 0;
	...
	ret = nand_read(&nand_info[0], i * nand_info[0].erasesize, &total, (u_char*)env_ptr);
    nandEnvBase = i * nand_info[0].erasesize;
#else
	ret = nand_read(&nand_info[0], CFG_ENV_OFFSET, &total, (u_char*)env_ptr);
#endif
    /* Set environment base offset variable to be used by Linux to locate the environment */
